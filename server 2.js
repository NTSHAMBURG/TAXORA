...
const ADMIN_CODE        = process.env.ADMIN_CODE || ""; // optional: zum Ansehen der Meldungen unter /reports
...
// ---- Problemmeldungen aus der App ----
const reports = [];            // im Speicher, die letzten 100
const reportHits = new Map();  // einfache IP-Bremse gegen Spam
function reportLimited(ip) {
  const now = Date.now();
  const arr = (reportHits.get(ip) || []).filter(t => now - t < 60000);
  if (arr.length >= 5) { reportHits.set(ip, arr); return true; }
  arr.push(now); reportHits.set(ip, arr); return false;
}
...
app.post("/report", (req, res) => {
  ...
  if (reportLimited(ip)) return res.status(429).json({ error: "Zu viele Meldungen. Bitte kurz warten." });
  const body = req.body || {};
  const message = (body.message || "").toString().slice(0, 2000).trim();
  if (!message) return res.status(400).json({ error: "Leere Meldung." });
  let context = {};
  try { context = JSON.parse(JSON.stringify(body.context || {})); } catch (_) { context = {}; }
  const entry = { time: new Date().toISOString(), ip, message, context };
  reports.unshift(entry);
  if (reports.length > 100) reports.length = 100;
  console.log("[TAXORA-REPORT] " + entry.time + " | " + (context.mandant || "?") + " | " + (context.geraet || "") + " | " + message);
  res.json({ ok: true });
});

app.get("/reports", (req, res) => {
  if (!ADMIN_CODE) return res.status(503).json({ error: "Reports-Ansicht nicht aktiviert (ADMIN_CODE nicht gesetzt)." });
  const got = (req.headers["x-admin-code"] || req.query.code || "").toString();
  if (got !== ADMIN_CODE) return res.status(401).json({ error: "Admin-Code ungültig." });
