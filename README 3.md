...
- `POST /report` — nimmt Problemmeldungen aus der App entgegen (kein Zugangscode nötig). Wird in den Logs ausgegeben und im Speicher gehalten.
- `GET /reports` — zeigt die letzten Meldungen (nur mit `ADMIN_CODE`).
...
| `ADMIN_CODE` | nein | — | Wenn gesetzt: erlaubt das Ansehen der Problemmeldungen unter `/reports`. |
